...
export const RainyDay = function (options, canvas) {
    this.img = options.image;
    const defaults = {
        opacity: 1,
        blur: 10,
        crop: [0, 0, options.width, options.height],
...
RainyDay.prototype.REFLECTION_MINIATURE = function (drop) {
    const sx = Math.max((drop.x - this.options.reflectionDropMappingWidth) / this.options.reflectionScaledownFactor, 0);
    const sy = Math.max((drop.y - this.options.reflectionDropMappingHeight) / this.options.reflectionScaledownFactor, 0);
    const sw = this.positiveMin(this.options.reflectionDropMappingWidth * 2 / this.options.reflectionScaledownFactor, this.reflected.width - sx);
    const sh = this.positiveMin(this.options.reflectionDropMappingHeight * 2 / this.options.reflectionScaledownFactor, this.reflected.height - sy);
