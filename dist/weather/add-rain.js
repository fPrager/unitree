...
var RainyDay = exports.RainyDay = function RainyDay(options, canvas) {
...
RainyDay.prototype.rain = function (presets, speed) {
    ...
    this.addDropCallback = function () {
        ...
        for (var _i = 0; _i < presets.length; _i++) {
            if (presets[_i][2] > 1 || presets[_i][3] === -1) {
                if (presets[_i][3] !== 0) {
                    presets[_i][3]--;
                    for (var y = 0; y < presets[_i][2]; ++y) {
                        this.putDrop(new Drop(this, Math.random() * this.canvas.width, Math.random() * this.canvas.height, presets[_i][0], presets[_i][1]));
                    }
                }
            } else if (Math.random() < presets[_i][2]) {
                preset = presets[_i];
                break;
            }
        }
